...
from keras.layers import Embedding, Dense, Input, LSTM, Bidirectional, Activation, CuDNNLSTM
...
class ModelBuilder:
    ...
    # model that takes the comment as input    
    ...
    # Model that takes parent_comment and comment as input
    ...
    # Model that takes parent_comment and comment as input
    # Built using CuDNNLST for GPU
    def multi_input_model_cudnn(self, pr_len, comm_len, optimizer = 'adam', 
                          loss='binary_crossentropy'):
        with K.name_scope('input_parent'):
            input_parent = Input(shape=(pr_len,), name='input_parent')
            embedding1 = Embedding(self.vocab_size, 100, 
        ...
        with K.name_scope('main_flow'):
            x = concatenate([embedding2, embedding1], axis=1, name='concat')
            x = Bidirectional(CuDNNLSTM(20, return_sequences=True, name='bilstim_1'))(x)
            x = Dense(20, activation='sigmoid', name='dense_1')(x)
            x = Bidirectional(CuDNNLSTM(20, name='bilstm_2'))(x)
            output = Dense(2, activation='sigmoid', name='output')(x)
